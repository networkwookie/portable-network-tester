...
import pytest
from testers.connectivity.models import Status
...
class TestConnectivityIntegration:
    ...
    def test_full_connectivity_suite_success(
        ...
        from testers.connectivity.models import (
            DHCPResult,
            DNSResult,
            LinkStatus,
            NeighborInfo,
        ...
        assert results["physical_link"].status == Status.PASSED
        assert results["neighbor_discovery"].status == Status.PASSED
        assert results["dhcp"].status == Status.PASSED
        assert results["dns"].status == Status.PASSED
        assert results["gateway_ping"].status == Status.PASSED
        assert results["internet_ping"].status == Status.PASSED
    ...
    def test_link_failure_skips_remaining_tests(self, mock_link):
        ...
        assert results["physical_link"].status == Status.FAILED

        # Verify other tests were skipped
        assert results["neighbor_discovery"].status == Status.PENDING
        assert results["dhcp"].status == Status.PENDING
    ...
    def test_progress_callback(self):
        ...
        with patch("testers.connectivity.link.LinkTester.test_link") as mock_link:
            from testers.connectivity.models import LinkStatus
