...
class MenuScreen(Screen):
    ...
    def build_ui(self) -> None:
        ...
        btn_speedtest = self._create_menu_button(
            "Speed Test", "Measure network bandwidth and latency", self.goto_speedtest
        )
        layout.add_widget(btn_speedtest)

        btn_capture = self._create_menu_button(
            "Packet Capture", "Capture and analyze network traffic", self.goto_capture
        )
        layout.add_widget(btn_capture)

        # Exit button
        btn_exit = Button(
            text="Exit", size_hint=(1, 0.15), background_color=(0.8, 0.2, 0.2, 1), font_size="20sp"
        )
